...
- **Команда расширенного анализа** `advanced_analysis` с сохранением результатов в БД
- **Модели для хранения результатов анализа**: QueryAnalysisResult, TableAnalysis, IndexRecommendation
- **Автоматическое определение типов запросов** и обработка INSERT ... SELECT
- **Извлечение SELECT части** из сложных запросов с CTE для анализа через EXPLAIN
- **Сохранение статистики таблиц** и рекомендаций по индексам
- **Обработка ошибок анализа** с сохранением информации об ошибках

### Улучшения анализа запросов

- **Определение типа SQL запроса** (SELECT, INSERT, UPDATE, DELETE, CREATE, etc.)
- **Извлечение SELECT части** из INSERT ... SELECT запросов для анализа
- **Поддержка CTE (Common Table Expressions)** в анализируемых запросах
- **Автоматический пропуск не-SELECT запросов** с сохранением базовой информации
- **Опция `--analyze-non-select`** для принудительного анализа всех типов запросов

### Сохранение результатов

- **QueryAnalysisResult** - полные результаты анализа с метриками сложности
- **TableAnalysis** - детальная статистика по используемым таблицам
- **IndexRecommendation** - автоматические рекомендации по созданию индексов
- **Связь с QueryLog** - результаты привязаны к исходным логам запросов

### Команда управления

- **`advanced_analysis`** - комплексный анализ с EXPLAIN и сохранением результатов
- **Параметры**: `--query-id`, `--limit`, `--force-reanalyze`, `--save-tables`, `--analyze-non-select`
- **Интеграция с ClickHouseClient** - использование существующей системы подключений
...
- **Этап 5.3**: Интеграция с реальными метриками выполнения
...
### Критерии успеха Этапа 5.2

- ✅ Команда расширенного анализа с сохранением результатов
- ✅ Модели для хранения QueryAnalysisResult, TableAnalysis, IndexRecommendation
- ✅ Автоматическое определение типов запросов и обработка INSERT ... SELECT
- ✅ Извлечение SELECT части из сложных запросов с CTE
- ✅ Сохранение статистики таблиц и рекомендаций по индексам

### Следующие шаги

**Этап 5.3: Интеграция с реальными метриками выполнения** - сравнение результатов анализа с фактическими метриками
выполнения, создание системы валидации рекомендаций, интеграция с историческими данными производительности.